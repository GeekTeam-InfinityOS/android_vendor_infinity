...
- githubremote:    Add git remote for InfinityOS Github.
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop top.infinity.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/GeekTeam-InfinityOS/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop top.infinity.device | grep -q "$CUSTOM_BUILD");
    then
